# styleguide
```sh
npm install order66-styleguide
```
...
```sh
@import './node_modules/order66-styleguide/styleguide.scss';
```
...
```sh
npm update --save order66-styleguide
npm run scss
```
# API Usage
## ListApiPlugin 
#### in main.js file:
```sh
import listApiPlugin from 'order66-styleguide/js/plugin/ListApiPlugin.js'
```
```sh
const listConfigs = [
...
#### in main.js file:
```sh
import userApiPlugin from 'order66-styleguide/js/plugin/UserApiPlugin.js'
```
```sh
const userConfigs = [
...
#### in main.js file:
```sh
import legacyUserApiPlugin from 'order66-styleguide/js/plugin/LegacyUserApiPlugin.js'
```
```sh
...
```
you need a contributor npm account to run these, verify on https://www.npmjs.com/package/order66-styleguide/access
``sh
npm publish
```
